// integrationTest.js 
// Ref: Asynchronous Requests Lecture - CS 290

class integrationTest {
    testEmployeeBE() {

        // view-employee
        var req = new XMLHttpRequest();
        req.open('GET', 'http://flip1.engr.oregonstate.edu:65351/view-employee', true);
        req.setRequestHeader('Content-Type', 'application/json');
        req.addEventListener('load', function () {
            if (req.status >= 200 && req.status < 400) {
		console.log('/view-employee: status 200 - Passed'); 	
                var result = req.response;
                var r, key;
                // from unit test for getEmployee(), implemented here as we've added another wraper around getEmpoyee:
                if (result.length > 0) {
                    console.log('/view-employee: Result length > 0 - Passed');
                }
                else {
                    console.log('/view-employee: Result length <= 0 - Failed');
                }
                result = JSON.parse(result);
                for (r in result) {
                    for (key in result[r]) {
                        if (key == 'groupName' || key == 'firstName' || key == 'lastName' || key == 'groupId') {
                            console.log('/view-employee: Result attribute is ' + key + ' - Passed');
                        }
                        else {
                            console.log('/view-employee: Result attribute is ' + key + ' - Failed');
                        }
                    }
                }
            }
            else {
                console.log("ERROR: " + req.statusText);

            }
        });
        req.send(null);

        // add-employee - happy path 
        var req2 = new XMLHttpRequest();
        req2.open('POST', 'http://flip1.engr.oregonstate.edu:65351/add-employee', true);
        req2.setRequestHeader('Content-Type', 'application/json');
        var payload = {};
        payload.body = {};
        payload.body.first = 'Trevor';
        payload.body.last = 'Worthey';
        payload.body.group_selected = 'Group2';

        req2.addEventListener('load', function () {
            if (req2.status >= 200 && req2.status < 400) {
                console.log('/add-employee: status 200 - Passed')
            }
            else {
                console.log("ERROR: " + req2.statusText);
            }
        });
        req2.send(payload);



        // add-group - happy path 
        var group = 'group1';

        var req3;
        req3 = new XMLHttpRequest();
        req3.open('POST', 'http://flip1.engr.oregonstate.edu:65351/add-group', true);
        req3.setRequestHeader('Content-Type', 'application/json');
        var payload = {};
        payload.body = {};
        payload.body.group_name = group;

        req3.addEventListener('load', function () {
            if (req3.status >= 200 && req3.status < 400) {
                console.log('/add-group: status 200 - Passed')
            }
            else {
                console.log("ERROR: " + req2.statusText);
            }
        });
        req3.send(payload);
    }

}
 	
it = new integrationTest();
it.testEmployeeBE();
