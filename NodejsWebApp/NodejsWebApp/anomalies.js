...
// Constants
const ANOMALIES_DEFAULT_QUERY = "SELECT anomalies.id, anomalies.timestamp, employees.f_name AS Employee, groups.name AS Employee_Group, anomaly_types.name AS Type, anomalies.latitude, anomalies.longitude FROM anomalies INNER JOIN employees ON anomalies.employee_id = employees.id INNER JOIN groups ON anomalies.group_id = groups.id INNER JOIN anomaly_types ON anomalies.type_id = anomaly_types.id";
const ANOMALIES_TYPES_QUERY = "SELECT anomalies.id, anomalies.timestamp, employees.f_name AS Employee, groups.name AS Employee_Group, anomaly_types.name AS Type, anomalies.latitude, anomalies.longitude FROM anomalies INNER JOIN employees ON anomalies.employee_id = employees.id INNER JOIN groups ON anomalies.group_id = groups.id INNER JOIN anomaly_types ON anomalies.type_id = anomaly_types.id ORDER BY Type ASC";
const ANOMALIES_GROUPS_QUERY = "SELECT anomalies.id, anomalies.timestamp, employees.f_name AS Employee, groups.name AS Employee_Group, anomaly_types.name AS Type, anomalies.latitude, anomalies.longitude FROM anomalies INNER JOIN employees ON anomalies.employee_id = employees.id INNER JOIN groups ON anomalies.group_id = groups.id INNER JOIN anomaly_types ON anomalies.type_id = anomaly_types.id ORDER BY Employee_Group ASC";
const ANOMALIES_EMPLOYEES_QUERY = "SELECT anomalies.id, anomalies.timestamp, employees.f_name AS Employee, groups.name AS Employee_Group, anomaly_types.name AS Type, anomalies.latitude, anomalies.longitude FROM anomalies INNER JOIN employees ON anomalies.employee_id = employees.id INNER JOIN groups ON anomalies.group_id = groups.id INNER JOIN anomaly_types ON anomalies.type_id = anomaly_types.id ORDER BY employees.f_name ASC";
